...
def erstelle_ticket_notiz():
    ...
    notizinhalt = vorlage.format(name=name, ticket_id=ticket_id, prioritaet=prioritaet, kategorie=kategorie, nebenkategorie=nebenkategorie)
...
# Kategorie
tk.Label(root, text="Kategorie:").grid(row=2, column=0, padx=10, pady=10)
kategorie_var = ttk.Combobox(root, values=["Arbeit", "Organisation", "Hobby", "Haushalt", "Routine"], width=37)
kategorie_var.grid(row=2, column=1, padx=10, pady=10)

# Nebenkategorie
tk.Label(root, text="Nebenkategorie:").grid(row=3, column=0, padx=10, pady=10)
nebenkategorie_entry = tk.Entry(root, width=40)
nebenkategorie_entry.grid(row=3, column=1, padx=10, pady=10)

# Vorlagen auswählen
vorlagen = lade_vorlagen()
vorlagen_namen = list(vorlagen.keys())

tk.Label(root, text="Vorlage auswählen:").grid(row=4, column=0, padx=10, pady=10)
vorlagen_combobox = ttk.Combobox(root, values=vorlagen_namen, width=37)
vorlagen_combobox.grid(row=4, column=1, padx=10, pady=10)

# Button zum Erstellen des Tickets
tk.Button(root, text="Ticket erstellen", command=erstelle_ticket_notiz).grid(row=5, column=0, columnspan=2, pady=20)
