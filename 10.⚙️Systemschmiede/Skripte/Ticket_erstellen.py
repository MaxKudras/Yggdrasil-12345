...
def erstelle_ticket_notiz():
    ...
    ticketbearbeitungsdatum = cal.get_date()
    ...
    notizinhalt = vorlage.format(name=name, ticket_id=ticket_id, prioritaet=prioritaet, kategorie=kategorie, ticketbearbeitungsdatum=ticketbearbeitungsdatum)
...
def open_calendar():
    ...
    global cal
    cal = Calendar(top, selectmode='day', year=2025, month=2, day=1)
    cal.pack(pady=20)
    tk.Button(top, text="Datum übernehmen", command=top.destroy).pack()
...
prioritaet_var = ttk.Combobox(root, values=["Kritisch","Hoch", "Mittel", "Niedrig"], width=37)
...
# Ticketbearbeitungsdatum
tk.Label(root, text="Ticketbearbeitungsdatum:").grid(row=4, column=0, padx=10, pady=10)
tk.Button(root, text="Datum wählen", command=open_calendar).grid(row=4, column=1, padx=10, pady=10)
