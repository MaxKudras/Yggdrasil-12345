...
var IndexNotesSettingTab = class extends import_obsidian2.PluginSettingTab {
  ...
  add_exclude_folders_setting() {
    new import_obsidian2.Setting(this.containerEl).setName("Excluded folders from indexing").setHeading();
...
var IndexSchema = class {
  constructor() {
    this.indexNotes = [];
    this.indexNoteContents = /* @__PURE__ */ new Map();
  }
  // filepath -> content hash
  getHash() {
    const structureHash = this.rootNode.getHash() + this.indexNotes.map((n) => n.getHash()).join();
    const contentHashes = Array.from(this.indexNoteContents.entries()).sort(([a], [b]) => a.localeCompare(b)).map(([path, contentHash]) => `${path}:${contentHash}`).join("|");
...
var IndexUpdater = class {
  ...
  constructor(app, settings) {
    ...
    this.recentlyModifiedFiles = /* @__PURE__ */ new Map();
    this.app = app;
    this.settings = settings;
  ...
  onFileModified(file) {
    ...
    const frontmatter = (_a = this.app.metadataCache.getCache(file.path)) == null ? void 0 : _a.frontmatter;
    const fileTags = (frontmatter == null ? void 0 : frontmatter.tags) || [];
    const hasIndexTags = Array.isArray(fileTags) ? fileTags.some(
      (tag) => tag.endsWith(`/${this.settings.index_tag}`) || tag.endsWith(`/${this.settings.meta_index_tag}`)
    ) : false;
    if (hasIndexTags) {
      this.scheduleUpdate(3e3);
    } else {
  ...
  async update() {
    ...
    try {
      ...
      const cutoffTime = Date.now() - 5e3;
      for (const [filepath, timestamp2] of this.recentlyModifiedFiles.entries()) {
  ...
  async processIndexNoteWithRetry(indexNote, indexBlocks, maxRetries = 3) {
    ...
    if (recentModificationTime) {
      ...
      if (timeSinceModification < 2e3) {
        const additionalDelay = 2e3 - timeSinceModification;
    ...
    throw new Error(`Failed to update index after ${maxRetries} attempts: ${lastError == null ? void 0 : lastError.message}`);
  }
  startHealthCheck() {
...
var IndexNotesPlugin = class extends import_obsidian3.Plugin {
  ...
  reset_update_interval() {
    window.clearInterval(this.update_interval_id);
    this.update_interval_id = window.setInterval(() => this.index_updater.update(), 12e4);
  }
  ...
  async newNoteFromFocusedFile() {
    ...
    let file_tags = [];
    if ((_c = metadata_cache == null ? void 0 : metadata_cache.frontmatter) == null ? void 0 : _c.tags) {
...
var PromptModal = class extends import_obsidian3.Modal {
  ...
  enterCallback(evt) {
    var _a;
    try {
      if (evt.key === "Enter" && ((_a = this.result) == null ? void 0 : _a.length)) {
        this.close();
