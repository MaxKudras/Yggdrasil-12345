...
var TimelineCalculator = class {
  ...
  getParentTasksToUpdate(_tasks, changedTask) {
...
var FileSync = class {
  ...
  async updateTask(filePath, task, updates, _globalStartDate) {
    const file = this.app.vault.getAbstractFileByPath(filePath);
    if (!(file instanceof import_obsidian.TFile))
      return false;
    try {
...
var GanttRenderer = class {
  ...
  renderGrid(state, width, height) {
    ...
    dates.forEach((date, index) => {
      ...
      if (state.zoomLevel === "day" && isWeekend(date)) {
        const weekendBg = createSvg("rect");
  ...
  renderHeader(state, width) {
    const headerGroup = createSvg("g");
    headerGroup.setAttribute("class", "gantt-header");
    const headerBg = createSvg("rect");
    ...
    dates.forEach((date, index) => {
      ...
      if (state.zoomLevel === "day" && date.getDate() === 1) {
        const monthText = createSvg("text");
  ...
  renderTodayLine(state, height) {
    ...
    const headerRect = createSvg("rect");
  ...
  renderProjectHeader(group, project, rowIndex, state, projectIndex) {
    ...
    const dragHandle = createSvg("text");
    ...
    if (hasTasksToToggle) {
      const collapseIcon = createSvg("text");
  ...
  renderProjectBar(group, project, y, state) {
    ...
    const durationText = createSvg("text");
  ...
  renderTaskRow(group, task, state, rowIndex, indentLevel) {
    ...
    if (task.indexInProject > 0 && task.level < 3) {
      const indentBtn = createSvg("text");
    ...
    if (task.level > 1) {
      const outdentBtn = createSvg("text");
    ...
    const deleteBtn = createSvg("text");
  ...
  renderTaskBar(group, task, x, y, width, height, state) {
    ...
    const leftHandle = createSvg("rect");
    ...
    const rightHandle = createSvg("rect");
    ...
    const isCompleted = task.status === "done" || task.isDone;
    const isCancelled = task.status === "cancelled";
    const taskNameLabel = createSvg("text");
  ...
  renderMilestone(group, task, x, y, state) {
    ...
    const isCompleted = task.status === "done" || task.isDone;
    const isCancelled = task.status === "cancelled";
    const milestoneLabel = createSvg("text");
  ...
  startDragReorder(taskId, e) {
    ...
    this.dragReorderIndicator = createSvg("line");
    ...
    activeDocument.body.addClass("is-grabbing");
  ...
  handleDragReorderMove(e) {
    ...
    this.taskRowPositions.forEach((pos, _taskId) => {
  ...
  endDragReorder(_e) {
    activeDocument.body.removeClass("is-grabbing");
    if (this.svg) {
  ...
  startProjectDragReorder(projectId, _e) {
    if (!this.svg || !this.state)
      return;
...
var KanbanRenderer = class {
  ...
  createCard(task, project, state) {
    ...
    duration.appendChild(activeDocument.createTextNode(` ${task.duration}d`));
    meta.appendChild(duration);
    if (task.dependencies.length > 0) {
      const deps = createSpan();
      deps.className = "kanban-meta-item";
      deps.appendChild(createLinkIcon());
      deps.appendChild(activeDocument.createTextNode(` ${task.dependencies.length}`));
    ...
    card.addEventListener("dragend", () => {
      ...
      this.draggedTaskId = null;
      activeDocument.querySelectorAll(".kanban-column").forEach((col) => {
...
var DragHandler = class {
  ...
  startDrag(taskId, e, dragType) {
    ...
    activeDocument.body.addClass("is-dragging");
    e.preventDefault();
  }
  cleanup() {
    ...
    activeDocument.body.removeClass("is-dragging");
    this.state = {
      isDragging: false,
      dragType: null,
      taskId: null,
  ...
  updateVisualFeedback(_newStartDate, _newDuration) {
...
var Toolbar = class {
  ...
  renderLogo(container) {
    ...
    const textSpan = createSpan({ cls: "toolbar-logo-text" });
    const strong = createEl("strong", { text: "Runalone" });
    textSpan.appendChild(strong);
    textSpan.appendChild(activeDocument.createTextNode(" project manager"));
...
var ProjectEditModal = class extends import_obsidian3.Modal {
  ...
  onOpen() {
    ...
    PROJECT_ICONS.forEach((icon) => {
      const iconBtn = iconContainer.createSpan({
        text: icon,
