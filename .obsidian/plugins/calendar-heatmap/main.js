...
function getWeeklyNoteSettings() {
    try {
        ...
        const calendarSettings = pluginManager.getPlugin("calendar-heatmap")?.options;
...
class CalendarSettingsTab extends obsidian.PluginSettingTab {
    ...
    display() {
        ...
        if (!appHasDailyNotesPluginLoaded_1()) {
            this.containerEl.createDiv("settings-banner", (banner) => {
                banner.createEl("h3", {
                    text: "Daily Notes 插件未启用"
                });
                banner.createEl("p", {
                    cls: "setting-item-description",
                    text: "建议配合 Daily Notes 或 Periodic Notes 插件使用，以便创建和打开每日笔记。"
                });
            });
        }
        ...
        if (this.plugin.options.showWeeklyNote &&
            ...
            this.containerEl.createEl("p", {
                cls: "setting-item-description",
                text: "提示：周记设置未来建议迁移到 Periodic Notes 插件中。"
            });
    ...
    addHeatmapMetricSetting() {
        new obsidian.Setting(this.containerEl)
            .setName("热力图显示依据")
            .setDesc("选择颜色深浅按当天创建文件数量，还是按当天创建文件的总字数。创建文件数量不含 Daily Note，新增字数包含 Daily Note。")
            .addDropdown((dropdown) => {
            dropdown.addOption("words", "新增字数");
            dropdown.addOption("count", "创建文件数量（不含 Daily Note）");
            dropdown.setValue(this.plugin.options.heatmapMetric || "words");
    ...
    addWeekStartSetting() {
        ...
        new obsidian.Setting(this.containerEl)
            .setName("每周起始日")
            .setDesc("选择日历从星期几开始。选择“跟随系统”会使用 Moment.js 的默认设置。")
            .addDropdown((dropdown) => {
    ...
    addConfirmCreateSetting() {
        new obsidian.Setting(this.containerEl)
            .setName("创建新笔记前确认")
            .setDesc("点击不存在的日期时，先弹出确认窗口再创建每日笔记。")
            .addToggle((toggle) => {
    ...
    addLocaleOverrideSetting() {
        ...
        new obsidian.Setting(this.containerEl)
            .setName("覆盖语言环境")
            .setDesc("如果想使用不同于系统默认的语言环境，可以在这里设置。")
            .addDropdown((dropdown) => {
    ...
    addFontSizeSetting() {
        ...
        setting.addSlider((sliderComponent) => {
            ...
            slider
                .setLimits(8, 24, 1)
                .setValue(this.plugin.options.fontSize || 14)
                .onChange(async (value) => {
                textField.setValue(String(value));
                this.plugin.writeOptions(() => ({ fontSize: value }));
            });
        });
        // 添加文本输入框显示当前值
        setting.addText((textfield) => {
            textField = textfield;
            textfield.inputEl.style.width = "60px";
            textfield.setValue(String(this.plugin.options.fontSize || 14));
            textfield.onChange(async (value) => {
                const numValue = parseInt(value) || 14;
                const clampedValue = Math.max(8, Math.min(24, numValue));
                slider.setValue(clampedValue);
                this.plugin.writeOptions(() => ({ fontSize: clampedValue }));
            });
        });
    }
}
...
function getDateUIDFromFile(file) {
    ...
    // 直接使用正则表达式匹配 YYYY-MM-DD 格式
    const dateMatch = fileName.match(/^(\d{4}-\d{2}-\d{2})/);
    if (dateMatch) {
        const dateStr = dateMatch[1];
        const date = window.moment(dateStr, "YYYY-MM-DD", true);
        if (date.isValid()) {
    ...
    vault.getMarkdownFiles().forEach((file) => {
        ...
        if (dateMatch) {
            ...
            if (date.isValid()) {
                ...
                dailyNotes[dateUID] = file;
            }
        }
    });
    return dailyNotes;
}
function getCustomDailyNotesByDate(date, dailyNotes) {
    const dateUID = date.format("YYYY-MM-DD");
...
function generateCalendarContent(dailyNotes) {
    ...
    // 按年月分组
    const groupedByYear = {};
    for (const dateUID of sortedDates) {
        const file = dailyNotes[dateUID];
        // 从 dateUID 中提取年月信息（假设格式为 YYYY-MM-DD）
        const [year, month] = dateUID.split('-');
        ...
        groupedByYear[year][month].push(file);
    }
    // 按年倒序生成内容
    ...
    for (const year of years) {
        content += `## ${year}\n\n`;
        // 按月倒序
        const months = Object.keys(groupedByYear[year]).sort().reverse();
        for (const month of months) {
            content += `### ${year}-${month}\n\n`;
            // 文件按日期倒序排列
            const files = groupedByYear[year][month];
...
function createDailyNotesStore() {
    ...
    return Object.assign({ reindex: async () => {
            try {
                const customDailyNotes = getCustomDailyNotes(); // 您需要实现这个函数
                store.set(customDailyNotes);
                hasError = false;
                //更新Calendar的索引文件
                await generateCalendarIndex();
            }
            catch (err) {
                if (!hasError) {
...
function create_fragment$2(ctx) {
	...
	let h3;
	let span0;
	let t0_value = /*displayedMonth*/ ctx[0].format("M月") + "";
	let t0;
	let t1;
	let span1;
	let t2_value = "";
	...
	return {
		...
		p(new_ctx, [dirty]) {
			ctx = new_ctx;
			if ((!current || dirty & /*displayedMonth*/ 1) && t0_value !== (t0_value = /*displayedMonth*/ ctx[0].format("M月") + "")) set_data(t0, t0_value);
			if ((!current || dirty & /*displayedMonth*/ 1) && t2_value !== (t2_value = "")) set_data(t2, t2_value);
...
function add_css() {
	...
	style.id = "svelte-1y2tp8m-style";
	// 删除以下刷新按钮相关的样式
	style.textContent = ".calendar-controls.svelte-1y2tp8m.svelte-1y2tp8m{position:relative}"; // 只保留这一行，删除其他
...
function create_fragment(ctx) {
	...
	return {
		c() {
			...
			attr$1(div, "class", "calendar-controls svelte-1y2tp8m");
			attr$1(div, "style", /*calendarStyle*/ ctx[11]);
		},
		m(target, anchor) {
		...
		p(new_ctx, [dirty]) {
			...
			if (dirty & /*$activeFile*/ 4096) calendarbase_changes.selectedId = /*$activeFile*/ ctx[12];
			...
			if (!current || dirty & /*calendarStyle*/ 2048) {
				attr$1(div, "style", /*calendarStyle*/ ctx[11]);
			}
		},
		i(local) {
...
function instance($$self, $$props, $$invalidate) {
	...
	$$self.$$set = $$props => {
		...
		if ("onRefresh" in $$props) $$invalidate(8, onRefresh = $$props.onRefresh);
	};
	...
	$$self.$$.update = () => {
		...
		if ($$self.$$.dirty & /*$settings*/ 512) {
			$$invalidate(11, calendarStyle = `font-size: ${$settings.fontSize || 14}px;`);
		}
	};
...
class Calendar extends SvelteComponent$1 {
	constructor(options) {
		super();
		if (!document.getElementById("svelte-1y2tp8m-style")) add_css();
...
const streakSource = {
    getDailyMetadata: async (date) => {
        const file = getCustomDailyNotesByDate(date, get_store_value(dailyNotes));
        return {
...
function isHeatmapDailyNote(file) {
    const dateMatch = file.basename.match(/^(\d{4}-\d{2}-\d{2})/);
    if (!dateMatch || !window.moment(dateMatch[1], "YYYY-MM-DD", true).isValid()) {
        return false;
    }
    const folderPath = getHeatmapDailyNoteFolderPath();
    const filePath = obsidian__default['default'].normalizePath(file.path);
    if (!folderPath) {
...
async function buildCreatedNotesByDay() {
    ...
    await Promise.all(files.map(async (file) => {
        ...
        const dayKey = window.moment(file.stat.ctime).format("YYYY-MM-DD");
...
async function getCreatedNotesByDay() {
    if (!createdNotesHeatmapCache) {
...
function getCreatedWordsHeatmapLevel(wordCount) {
    if (!wordCount) {
        return 0;
    }
    if (wordCount <= 300) {
...
const createdNotesHeatmapSource = {
    getDailyMetadata: async (date) => {
        const dateKey = date.format("YYYY-MM-DD");
        const stat = (await getCreatedNotesByDay())[dateKey] || { count: 0, words: 0 };
        const mode = get_store_value(settings).heatmapMetric === "count" ? "count" : "words";
        const level = mode === "count" ? getCreatedNotesHeatmapLevel(stat.count) : getCreatedWordsHeatmapLevel(stat.words);
        const tooltip = `创建 ${stat.count} 篇笔记，新增 ${stat.words} 字`;
        return {
            classes: [`heatmap-level-${level}`],
            dataAttributes: {
                "data-created-note-count": String(stat.count),
                "data-created-word-count": String(stat.words),
...
class CalendarView extends obsidian.ItemView {
    ...
    async onOpen() {
        // Integration point: external plugins can listen for `calendar-heatmap:open`
        // to feed in additional sources.
        const sources = [
            createdNotesHeatmapSource,
            customTagsSource,
            streakSource,
        ];
        ...
        this.calendar = new Calendar({
            ...
            props: {
                ...
                onRefresh: this.refreshFileIndex.bind(this),
                sources,
            },
        });
    }
    onHoverDay(date, targetEl, isMetaPressed) {
        ...
        const { format } = getDailyNoteSettings_1();
        const note = getCustomDailyNotesByDate(date, get_store_value(dailyNotes));
    ...
    async openOrCreateDailyNote(date, inNewSplit) {
        const { workspace } = this.app;
        const existingFile = getCustomDailyNotesByDate(date, get_store_value(dailyNotes));
        if (!existingFile) {
            // File doesn't exist
