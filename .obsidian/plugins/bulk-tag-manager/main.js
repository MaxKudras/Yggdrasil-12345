...
var TagLowercasePlugin = class extends import_obsidian.Plugin {
  ...
  registerTagWranglerFeatures() {
    ...
    (_a = workspaceWithHoverSources.registerHoverLinkSource) == null ? void 0 : _a.call(workspaceWithHoverSources, TAG_HOVER_SOURCE, { display: "Tags View", defaultMod: true });
  ...
  updateTagPage(file, frontmatter) {
    var _a, _b, _c, _d;
    const tags = ((_b = (_a = (0, import_obsidian.parseFrontMatterAliases)(frontmatter)) == null ? void 0 : _a.filter((alias) => /^#[^\s]+$/.test(alias))) != null ? _b : []).map((alias) => alias.replace(/^##+/, "#"));
    ...
    new BtmConfirmationModal(
      ...
      async () => {
        ...
        try {
          ...
          new import_obsidian.Notice(`Finished: ${changes.length} files changed. ${errors.length > 0 ? `(${errors.length} skipped due to errors)` : ""}`);
          if (errors.length > 0) {
...
`;
  ...
  async nestTags(parent, children) {
    ...
    if (missingTags.length > 0) {
      new import_obsidian.Notice(`Nest aborted: ${missingTags.join(", ")} not found in current scope. Check for typos or scope filters.`);
      return;
    }
  ...
  async renameTagBatch(pairs) {
    ...
    if (missingTags.length > 0) {
      new import_obsidian.Notice(`Warning: ${missingTags.length} source tag(s) not found in scope: ${missingTags.slice(0, 5).map((t) => "#" + t).join(", ")}${missingTags.length > 5 ? "\u2026" : ""}. Continuing with found tags.`);
    }
...
var BulkManagerSettingsDashboard = class {
  ...
  render() {
    ...
    new import_obsidian.Setting(settingsBox).setName("Case Strategy").addDropdown((dropdown) => dropdown.addOption("lowercase", "Lowercase").addOption("uppercase", "Uppercase").addOption("none", "No Change").setValue(this.plugin.settings.caseStrategy).onChange((value) => {
      runAsync(async () => {
    ...
    new import_obsidian.Setting(settingsBox).setName("Separator Style").addDropdown((dropdown) => dropdown.addOption("preserve", "Preserve").addOption("snake", "Snake Case").addOption("kebab", "Kebab Case").setValue(this.plugin.settings.separatorStrategy).onChange((value) => {
      runAsync(async () => {
    ...
    new import_obsidian.Setting(settingsBox).setName("Remove Special Characters").setDesc("Removes everything except letters, numbers, hyphens (-), and underscores (_).").addToggle((toggle) => toggle.setValue(this.plugin.settings.removeSpecialChars).onChange((value) => {
      runAsync(async () => {
    ...
    new import_obsidian.Setting(settingsBox).setName("Flatten Diacritics").setDesc("Converts accented characters to their plain equivalents.").addToggle((toggle) => toggle.setValue(this.plugin.settings.flattenDiacritics).onChange((value) => {
      runAsync(async () => {
    ...
    new import_obsidian.Setting(settingsBox).setName("Apply to Nested Tags").addToggle((toggle) => toggle.setValue(this.plugin.settings.applyToNestedTags).onChange((value) => {
      runAsync(async () => {
    ...
    new import_obsidian.Setting(settingsBox).setName("Enable Scope Filter").setDesc("Limit operations to specific folders").addToggle((toggle) => toggle.setValue(this.plugin.settings.scopeFilter.enabled).onChange((value) => {
      runAsync(async () => {
    ...
    new import_obsidian.Setting(settingsBox).setName("File Pattern Filter").setDesc("Optional regex applied to file paths when the scope filter is enabled.").addText((text) => text.setPlaceholder("^Projects/").setValue(this.plugin.settings.scopeFilter.filePattern).onChange((value) => {
      runAsync(async () => {
...
var TagManagerModal = class extends import_obsidian.Modal {
  ...
  onOpen() {
    ...
    new import_obsidian.Setting(settingsBox).setName("Flatten Diacritics").setDesc("Converts accented characters to their plain equivalents (e.g., \xE1 \u2192 a, \xE5 \u2192 a).").addToggle((toggle) => toggle.setValue(this.plugin.settings.flattenDiacritics).onChange((value) => {
      runAsync(async () => {
        this.plugin.settings.flattenDiacritics = value;
        await this.plugin.saveSettings();
        this.updateStats().catch((e) => console.error("Failed to update stats", e));
...
var TagLowercaseSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian.Setting(historySection).setName("Max History Size").setDesc("Number of operations to keep in history").addSlider((slider) => slider.setLimits(10, 100, 10).setValue(this.plugin.settings.maxHistorySize).setDynamicTooltip().onChange((value) => {
      runAsync(async () => {
        this.plugin.settings.maxHistorySize = value;
        await this.plugin.saveSettings();
      });
    }));
    new import_obsidian.Setting(historySection).setName("History Expiration (Days)").setDesc("Automatically delete history older than this many days (0 to disable).").addSlider((slider) => slider.setLimits(0, 30, 1).setValue(this.plugin.settings.historyExpirationDays).setDynamicTooltip().onChange((value) => {
      runAsync(async () => {
        this.plugin.settings.historyExpirationDays = value;
        await this.plugin.saveSettings();
      });
    }));
    new import_obsidian.Setting(historySection).setName("Orphan Threshold").setDesc("Tags used fewer times than this are considered orphaned").addSlider((slider) => slider.setLimits(1, 10, 1).setValue(this.plugin.settings.orphanThreshold).setDynamicTooltip().onChange((value) => {
      runAsync(async () => {
        this.plugin.settings.orphanThreshold = value;
        await this.plugin.saveSettings();
      });
    }));
    new import_obsidian.Setting(historySection).setName("Clear History").setDesc("Remove all operation history").addButton((btn) => btn.setButtonText("Clear").setWarning().onClick(() => {
      runAsync(async () => {
        for (const op of this.plugin.settings.operationHistory) {
  ...
  renderAliases(container) {
    ...
    for (const [alias, canonical] of Object.entries(aliases)) {
      new import_obsidian.Setting(container).setName(`#${alias} \u2192 #${canonical}`).addButton((btn) => btn.setIcon("trash").setWarning().onClick(() => {
        runAsync(async () => {
  ...
  renderProtectedTags(container) {
    ...
    protectedTags.forEach((tag, index) => {
      new import_obsidian.Setting(container).setName(tag).addButton((btn) => btn.setIcon("trash").setWarning().onClick(() => {
        runAsync(async () => {
