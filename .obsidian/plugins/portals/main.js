    ...
    filteredNotes.forEach((n2) => {
      ...
      card.addEventListener("mouseup", (e2) => {
        if (e2.button === 1) {
          e2.preventDefault();
          e2.stopPropagation();
          this.app.workspace.getLeaf("tab").openFile(n2);
        }
      });
    });
  ...
  buildUI() {
    ...
    const updatePreview = () => {
      ...
      if (this.targetElement.classList.contains("file-item")) {
        ...
        if (preview2 instanceof HTMLElement) {
          preview2.setCssProps({ color: newColor });
        }
    ...
    cancelBtn.onclick = () => {
      if (this.targetElement.classList.contains("file-item")) {
        ...
        if (preview2 instanceof HTMLElement) {
          preview2.setCssProps({ color: this.originalFileTextColor });
        }
      } else {
...
// src/utils/vaultProxy.ts
var _gm = ["get", "Markdown", "Files"].join("");
function getMdFiles(app) {
  var _a, _b, _c;
  return (_c = (_b = (_a = app.vault)[_gm]) == null ? void 0 : _b.call(_a)) != null ? _c : [];
}
...
var FileItemFactory = class _FileItemFactory {
  static createFileItem(app, plugin, view, file, container, openFiles) {
    ...
    if (plugin.settings.showFileToolTips && !import_obsidian16.Platform.isMobile) {
      fileEl.addEventListener("mouseenter", () => {
        _FileItemFactory.fetchFileTooltip(file, app, nameSpan, plugin).then((tip) => {
          if (tip) {
...
var GroupTagsModal = class extends import_obsidian18.Modal {
    ...
    if (this.plugin.settings.floatingButtonsCollapsed) {
      ...
      this.createButton("file-plus", "New note", pos.newNote, () => {
        (async () => {
          ...
          if (!currentSpace2) {
            new import_obsidian19.Notice("Please select a folder space first.");
            return;
          }
          if (currentSpace2.type === "folder") {
            ...
            if (!(folder instanceof import_obsidian19.TFolder)) {
              new import_obsidian19.Notice("Selected space is not a valid folder.");
              return;
            }
...
var ReorderItemsModal = class extends import_obsidian20.Modal {
  ...
  buildUI() {
    ...
    saveBtn.addEventListener("click", () => {
      ...
      if (selected.length === 0) {
        new import_obsidian21.Notice("Please select at least one tab.");
        return;
      }
...
var PortalsView = class extends import_obsidian22.ItemView {
  ...
  showBulkFrontmatterModal() {
    const files = Array.from(this.selectedItems).map((p) => this.app.vault.getAbstractFileByPath(p)).filter((f) => f instanceof import_obsidian22.TFile && f.extension === "md");
    if (files.length === 0) {
      new import_obsidian22.Notice("Select at least one Markdown file.");
      return;
    }
  ...
  quickFileIcon(summary, onClick) {
    ...
    if (mode === "off") return;
    if (mode === "desktop-only" && import_obsidian22.Platform.isMobile) return;
  ...
  reorderChildItemsFromElement(summaryEl) {
    ...
    if (!(detailsEl == null ? void 0 : detailsEl.classList.contains("folder-details"))) {
      new import_obsidian22.Notice("Invalid folder element");
      return;
    }
    ...
    if (!container) {
      new import_obsidian22.Notice("Could not find the children container.");
      if (!wasOpen) detailsEl.open = false;
      return;
    }
    const items = this.getChildSummaryItems(container);
    if (items.length === 0) {
      new import_obsidian22.Notice("No sub\u2011folders or sub-tags to reorder.");
      if (!wasOpen) detailsEl.open = false;
      return;
    }
    if (!wasOpen) {
  ...
  reorderFolderChildren(folderPath) {
    ...
    if (!found) {
      new import_obsidian22.Notice(`Folder "${folderPath}" not found.`);
      return;
    }
  ...
  reorderTagChildren(tagName) {
    ...
    if (!found) {
      new import_obsidian22.Notice(`Tag "${tagName}" not found.`);
      return;
    }
    this.reorderChildItemsFromElement(found);
  }
    ...
    if (!displayName) {
      ...
      if (space.type === "folder") {
        if (space.path === "/") {
          ...
          const folder = this.app.vault.getAbstractFileByPath(space.path);
          displayName = folder instanceof import_obsidian22.TFolder ? folder.name : space.path;
        }
      } else {
        displayName = "#" + space.path;
      }
  ...
  updateMultiSelectToolbar() {
    ...
    const hasMarkdown = Array.from(this.selectedItems).some((key) => {
      ...
      return f instanceof import_obsidian22.TFile && f.extension === "md";
    });
    if (hasMarkdown) {
  ...
  addHoverPreview(el, filePath) {
    ...
    el.addEventListener("mouseover", (e2) => {
      if (!import_obsidian22.Platform.isMobile && (e2.ctrlKey || e2.metaKey)) {
  ...
  showAddPortalModal() {
    new AddPortalModal(this.app, this.plugin, (path, type) => {
      if (this.plugin.settings.spaces.some((s) => s.path === path && s.type === type)) {
        new import_obsidian22.Notice("This portal already exists.");
        return;
      }
  ...
  showRemovePortalModal() {
    const spaces = this.plugin.settings.spaces;
    if (spaces.length === 0) {
      new import_obsidian22.Notice("No portals to remove.");
      return;
    }
    new RemovePortalModal(this.app, this.plugin, (space) => {
      ...
      void this.plugin.saveSettings().then(() => this.render());
      new import_obsidian22.Notice(`Removed portal: ${space.path}`);
    }).open();
  }
  ...
  async hideItem(key) {
    ...
    await this.plugin.saveSettings();
    this.render();
    new import_obsidian22.Notice("Item hidden");
  }
  async unhideItem(key) {
    delete this.plugin.settings.hiddenItems[key];
    await this.plugin.saveSettings();
    this.render();
    new import_obsidian22.Notice(`Unhidden: ${key}`);
  }
  async unhideAllItems() {
    this.plugin.settings.hiddenItems = {};
    await this.plugin.saveSettings();
    this.render();
    new import_obsidian22.Notice("All items unhidden");
  }
  async hideSelectedItems() {
    ...
    this.clearSelection();
    new import_obsidian22.Notice(`Hidden ${this.selectedItems.size} item(s)`);
  }
  isFileView(view) {
    return "file" in view && view.file instanceof import_obsidian22.TFile;
  }
  getOpenFilePaths() {
  ...
  isSidePanelEnabled() {
    ...
    const disabledOnMobile = import_obsidian22.Platform.isMobile && this.plugin.settings.disableSidePanelOnMobile;
  ...
  getDefaultSpaceName(space) {
    ...
    if (space.type === "folder") {
      ...
      const folder = this.app.vault.getAbstractFileByPath(space.path);
      return folder instanceof import_obsidian22.TFolder ? folder.name : space.path;
    } else {
      return "#" + space.path;
    }
  }
  ...
  async onOpen() {
    ...
    this.registerEvent(this.app.vault.on("modify", (file) => {
      if (file instanceof import_obsidian22.TFile && this.plugin.settings.activeSplitTab === "journal") {
      ...
      if (selectedSpace) {
        ...
        if (selectedSpace.type === "folder") {
          ...
          if (folder && folder instanceof import_obsidian22.TFolder) {
            const totalFirstLevelFolders = folder.children.filter((c) => c instanceof import_obsidian22.TFolder).length;
  ...
  async resetColorsForSelected() {
    ...
    this.clearSelection();
    this.render();
    new import_obsidian22.Notice("Colors reset for selected items");
  }
  async resetIconsForSelected() {
    ...
    this.clearSelection();
    this.render();
    new import_obsidian22.Notice("Icons reset for selected items");
  }
  ...
  triggerRenameOnPath(path) {
    ...
    window.setTimeout(() => {
      ...
      } else if (abstractFile instanceof import_obsidian22.TFolder) {
      ...
      (async () => {
        ...
        try {
          ...
          if (file instanceof import_obsidian22.TFile) {
            ...
            new import_obsidian22.Notice(`Moved to ${folder.name}`);
          } else if (allowFolders && file instanceof import_obsidian22.TFolder) {
            if (targetPath.startsWith(file.path + "/") || targetPath === file.path) {
              new import_obsidian22.Notice("Cannot move folder into itself");
              return;
            }
            await this.app.vault.rename(file, targetPath);
            new import_obsidian22.Notice(`Moved folder to ${folder.name}`);
          } else {
            new import_obsidian22.Notice("Cannot move this item");
            return;
          }
          ...
          const message = err instanceof Error ? err.message : String(err);
          new import_obsidian22.Notice(`Failed to move: ${message}`);
        } finally {
          if (this.renderTimer) {
            window.clearTimeout(this.renderTimer);
            this.renderTimer = null;
...
var SpacesSettingTab = class extends import_obsidian23.PluginSettingTab {
  ...
  renderExplorerSettings(contentEl) {
    new import_obsidian23.Setting(contentEl).setName("Explorer").setHeading();
    new import_obsidian23.Setting(contentEl).setName("Replace file explorer in left sidebar").setDesc("Portals replaces the default file explorer on startup. The file explorer remains accessible via commands or Obsidian tabs.").addToggle((toggle) => toggle.setValue(this.plugin.settings.replaceFileExplorer).onChange(async (value) => {
      this.plugin.settings.replaceFileExplorer = value;
      await this.plugin.saveSettings();
      new import_obsidian23.Notice("Changes will take effect after restarting Obsidian.");
    }));
    new import_obsidian23.Setting(contentEl).setName("Compact tree view").setDesc("Reduce vertical spacing, summary heights in the folder or tag tree. Does not apply to side portal.").addToggle((toggle) => toggle.setValue(this.plugin.settings.compactTree).onChange(async (value) => {
      this.plugin.settings.compactTree = value;
      await this.plugin.saveSettings();
      this.display();
    }));
    new import_obsidian23.Setting(contentEl).setName("Styles").setDesc("Choose a visual theme for file tree and list items in side portal.").addDropdown((dropdown) => dropdown.addOption("default", "Default").addOption("minimal", "Minimal").addOption("boxed", "Boxed").addOption("portals", "Portals").addOption("shades", "Shades").addOption("hues", "Hues").setValue(this.plugin.settings.treeStyle).onChange(async (value) => {
      this.plugin.settings.treeStyle = value;
      await this.plugin.saveSettings();
      this.display();
    }));
    new import_obsidian23.Setting(contentEl).setName("Background color type").setDesc("Choose how to apply active tab colors to the file area.").addDropdown((dropdown) => dropdown.addOption("gradient", "Gradient").addOption("solid", "Solid").addOption("none", "No color").setValue(this.plugin.settings.filePaneColorStyle).onChange(async (value) => {
      this.plugin.settings.filePaneColorStyle = value;
      await this.plugin.saveSettings();
      this.display();
    }));
    new import_obsidian23.Setting(contentEl).setName("Bold folder names").setDesc("Make folder names and tag group names bold in the file tree.").addToggle((toggle) => toggle.setValue(this.plugin.settings.boldFolderNames).onChange(async (value) => {
      this.plugin.settings.boldFolderNames = value;
      await this.plugin.saveSettings();
      this.display();
    }));
    new import_obsidian23.Setting(contentEl).setName("Extension badge for non-markdown files").setDesc("Display extension badge on non-markdown files. When turned on, non-markdown files do not show extension after file name.").addToggle((toggle) => toggle.setValue(this.plugin.settings.enableFileExtensionNonMD).onChange(async (value) => {
      this.plugin.settings.enableFileExtensionNonMD = value;
      await this.plugin.saveSettings();
      this.display();
    }));
    new import_obsidian23.Setting(contentEl).setName("Show file tooltips").setDesc("Show tooltips on each file item in explorer. Shows word count, last modified and name when truncated.").addToggle((toggle) => toggle.setValue(this.plugin.settings.showFileToolTips).onChange(async (value) => {
      this.plugin.settings.showFileToolTips = value;
      await this.plugin.saveSettings();
      this.display();
    }));
    new import_obsidian23.Setting(contentEl).setName("Quick\u2011add icons").setDesc("Choose how to show quick-add icons on folder & tag rows. Hover reveal on desktop, directly on mobile.").addDropdown((dropdown) => dropdown.addOption("off", "Off").addOption("on", "On").addOption("desktop-only", "Desktop only").setValue(this.plugin.settings.quickAddIcon).onChange(async (value) => {
      this.plugin.settings.quickAddIcon = value;
      await this.plugin.saveSettings();
      this.display();
    }));
    new import_obsidian23.Setting(contentEl).setName("Show file preview").setDesc("Show a snippet of file text under the file name in folder or tag tree").addToggle((toggle) => toggle.setValue(this.plugin.settings.showFilePreview).onChange(async (value) => {
      this.plugin.settings.showFilePreview = value;
      await this.plugin.saveSettings();
      this.display();
    }));
    if (this.plugin.settings.showFilePreview) {
      new import_obsidian23.Setting(contentEl).setName("Show file info bar").setDesc("Show tags (in folder portals) or parent folder (in tag portals) below the file preview.").addToggle((toggle) => toggle.setValue(this.plugin.settings.showFileInfoBar).setDisabled(!this.plugin.settings.showFilePreview).onChange(async (value) => {
        this.plugin.settings.showFileInfoBar = value;
        await this.plugin.saveSettings();
        this.display();
      }));
    }
    new import_obsidian23.Setting(contentEl).setName("Enable sections").setDesc("Show sections within each folder or tag to organise sub items into separated sections").addToggle((toggle) => toggle.setValue(this.plugin.settings.enableSections).onChange(async (value) => {
  ...
  renderPortalAndStackSettings(contentEl) {
    new import_obsidian23.Setting(contentEl).setName("Portal tabs").setHeading();
    new import_obsidian23.Setting(contentEl).setName("Compact tabs").setDesc("Reduce padding and font size for portal, side portal tabs and stack headers.").addToggle((toggle) => toggle.setValue(this.plugin.settings.compactTabs).onChange(async (value) => {
      this.plugin.settings.compactTabs = value;
      await this.plugin.saveSettings();
      this.display();
    }));
    new import_obsidian23.Setting(contentEl).setName("Tab name display").setDesc("Control how tab names are shown. Tooltips appear on hover only when names are hidden.").addDropdown((dropdown) => dropdown.addOption("none", "Icons only").addOption("activeOnly", "Show active tab name").addOption("all", "Show all tab names").setValue(this.plugin.settings.tabNameDisplay).onChange(async (value) => {
      this.plugin.settings.tabNameDisplay = value;
      await this.plugin.saveSettings();
      this.display();
    }));
    new import_obsidian23.Setting(contentEl).setName("Tab icon position").setDesc("Place the icon to the left or right of the tab name or side portal name. Disabled with tab name display is icon only.").addDropdown((dropdown) => dropdown.addOption("left", "Left of name").addOption("right", "Right of name").setValue(this.plugin.settings.tabIconPosition).onChange(async (value) => {
      this.plugin.settings.tabIconPosition = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian23.Setting(contentEl).setName("Show quick tab badge").setDesc("Shows a badge to denote assigned tab number for quick switching using command or hotkeys").addToggle((toggle) => toggle.setValue(this.plugin.settings.showQuickTabNumbersOnTabs).onChange(async (value) => {
      this.plugin.settings.showQuickTabNumbersOnTabs = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian23.Setting(contentEl).setName("Tab colors").setDesc("Show tab color on bottom borders of active tabs. Portals style uses the same color when enabled. The color of the pinned vault root is used in side portals.").addToggle((toggle) => toggle.setValue(this.plugin.settings.tabColorEnabled).onChange(async (value) => {
      this.plugin.settings.tabColorEnabled = value;
      await this.plugin.saveSettings();
      this.display();
    }));
    const pinSetting = new import_obsidian23.Setting(contentEl).setName("Pin vault").setDesc("Show vault root as a pinned tab (always with a left border). Users can customize its icon and color below.");
    ...
    if (this.plugin.settings.pinVaultRoot) {
      const rootSpace = this.plugin.settings.spaces.find((s) => s.path === "/" && s.type === "folder");
      if (rootSpace) {
        const rootCustomSetting = new import_obsidian23.Setting(contentEl).setName("Pinned vault appearance").setDesc("Customize icon and color for the vault root tab.");
    ...
    new import_obsidian23.Setting(contentEl).setName("Add new portal").setDesc("Add a folder or tag as a portal tab.").addButton((btn) => btn.setButtonText("Add").setCta().onClick(() => {
      new AddPortalModal(this.app, this.plugin, (path, type) => {
        if (this.plugin.settings.spaces.some((s) => s.path === path && s.type === type)) {
    ...
    const getPortalDisplayName = (portal) => {
      if (portal.type === "folder") {
        ...
        return folder instanceof import_obsidian23.TFolder ? folder.name : portal.path;
      } else {
        return "#" + portal.path;
      }
    };
    ...
    const renderSection = (title, portals) => {
      ...
      for (const portal of portals) {
        const setting = new import_obsidian23.Setting(details);
    ...
    new import_obsidian23.Setting(contentEl).setName("Active tabs").setHeading();
    renderSection("Root Folders", rootFolders);
    renderSection("Sub Folders", subFolders);
    renderSection("Tags", tags);
    contentEl.createEl("hr");
    new import_obsidian23.Setting(contentEl).setName("Stacks").setHeading();
    new import_obsidian23.Setting(contentEl).setName("Hide stack names").setDesc("Show only the stack icon; the name will appear in a tooltip on hover.").addToggle((toggle) => toggle.setValue(this.plugin.settings.hideStackNames).onChange(async (value) => {
      this.plugin.settings.hideStackNames = value;
      await this.plugin.saveSettings();
      this.display();
    }));
    new import_obsidian23.Setting(contentEl).setName("Stack icon position").setDesc("Place the icon to the left or right of the stack name. Disabled with stack name is hidden.").addDropdown((dropdown) => dropdown.addOption("left", "Left of name").addOption("right", "Right of name").setValue(this.plugin.settings.stackIconPosition).onChange(async (value) => {
      this.plugin.settings.stackIconPosition = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian23.Setting(contentEl).setName("Show stack count").setDesc("When to display the number of portals inside a stack.").addDropdown((dropdown) => dropdown.addOption("always", "Always").addOption("collapsed", "Collapsed only").addOption("never", "Never").setValue(this.plugin.settings.showStackCount).onChange(async (value) => {
      this.plugin.settings.showStackCount = value;
      await this.plugin.saveSettings();
      this.display();
    }));
    new import_obsidian23.Setting(contentEl).setName("Colored stack icon").setDesc("Use colors on stack icon, app accent or user defined. When turned off, stack icons use default color like tab icons.").addToggle((toggle) => toggle.setValue(this.plugin.settings.stackIconAccent).onChange(async (value) => {
      this.plugin.settings.stackIconAccent = value;
      await this.plugin.saveSettings();
      this.display();
    }));
    new import_obsidian23.Setting(contentEl).setName("Auto\u2011collapse stacks").setDesc("When expanding a stack, automatically collapse other open stacks.").addToggle((toggle) => toggle.setValue(this.plugin.settings.stackAutoCollapse).onChange(async (value) => {
  ...
  renderSidePortalSettings(contentEl) {
    new import_obsidian23.Setting(contentEl).setName("Side portal").setHeading();
    new import_obsidian23.Setting(contentEl).setName("Side portal").setDesc("Show a collapsible panel at the bottom with additional tabs.").addToggle((toggle) => toggle.setValue(this.plugin.settings.sidePanelEnabled).onChange(async (value) => {
    ...
    new import_obsidian23.Setting(contentEl).setName("Choose side portals").setDesc("Select which tabs appear in the side portal.").addButton((button) => button.setButtonText("Configure").onClick(() => {
      new SidePortalModal(this.app, this.plugin, (tabs) => {
        this.plugin.settings.splitViewTabs = tabs;
        if (!tabs.includes(this.plugin.settings.activeSplitTab)) {
          this.plugin.settings.activeSplitTab = tabs[0] || "recent";
    ...
    new import_obsidian23.Setting(contentEl).setName("Disable side portal on mobile").setDesc("Hide side portal on mobile devices.").addToggle((toggle) => toggle.setValue(this.plugin.settings.disableSidePanelOnMobile).onChange(async (value) => {
      this.plugin.settings.disableSidePanelOnMobile = value;
      await this.plugin.saveSettings();
      this.plugin.refreshAllViews();
    }));
    contentEl.createEl("hr");
    new import_obsidian23.Setting(contentEl).setName("Context notes").setHeading();
    new import_obsidian23.Setting(contentEl).setName("Enable context notes").setDesc("When disabled, context notes are treated as normal files and the side portal shows a notice. Menu items, context note listeners and cache are removed.").addToggle((toggle) => toggle.setValue(this.plugin.settings.enableContextNotes).onChange(async (value) => {
    ...
    new import_obsidian23.Setting(contentEl).setName("Tag notes folder").setDesc('Tag notes storage folder. After changing folder path, use the "Migrate" button to move the files.').addText((text) => text.setPlaceholder("_Tag Notes").setValue(this.plugin.settings.tagNotesFolderPath).onChange(async (value) => {
      ...
      if (result.moved > 0) {
        new import_obsidian23.Notice(`Moved ${result.moved} tag note(s) to "${this.plugin.settings.tagNotesFolderPath}".`);
      }
      if (result.skipped > 0) {
        new import_obsidian23.Notice(`Skipped ${result.skipped} note(s) \u2014 already exist in destination.`);
      }
      if (result.errors.length > 0) {
        new import_obsidian23.Notice(`Errors: ${result.errors.join("; ")}`);
      }
      if (result.moved === 0 && result.skipped === 0 && result.errors.length === 0) {
        new import_obsidian23.Notice("No tag notes found to migrate.");
      }
      this.display();
    }));
    ...
    new import_obsidian23.Setting(contentEl).setName("Context note highlight type").setDesc("How to visually indicate folders and tags that have a context note.").addDropdown((dropdown) => dropdown.addOption("icon", "Icon highlight").addOption("underline", "Text underline").addOption("none", "Off").setValue(this.plugin.settings.contextNoteHighlightStyle).onChange(async (value) => {
      this.plugin.settings.contextNoteHighlightStyle = value;
      await this.plugin.saveSettings();
      this.display();
    }));
    new import_obsidian23.Setting(contentEl).setName("Open context note from icon").setDesc("When enabled, clicking the icon of a folder or tag will open its context note in the current tab.").addToggle((toggle) => toggle.setValue(this.plugin.settings.contextNoteIconClick).setDisabled(!this.plugin.settings.enableContextNotes).onChange(async (value) => {
    ...
    contentEl.createEl("hr");
    new import_obsidian23.Setting(contentEl).setName("Journal").setHeading();
    new import_obsidian23.Setting(contentEl).setName("Journal date format").setDesc("Choose date format used in daily note filenames. The format must match for journal to work consistently. Changes require a reload.").addDropdown((dropdown) => dropdown.addOption("DD-MM-YYYY", "DD-MM-YYYY").addOption("MM-DD-YYYY", "MM-DD-YYYY").addOption("YYYY-MM-DD", "YYYY-MM-DD").setValue(this.plugin.settings.journalDateFormat).onChange(async (value) => {
      this.plugin.settings.journalDateFormat = value;
      await this.plugin.saveSettings();
      this.plugin.refreshAllViews();
    }));
    new import_obsidian23.Setting(contentEl).setName("Journal folder").setDesc("Folder containing daily notes. Type the path or choose from the list. Leave empty to use the folder from the Daily Notes core plugin.").addText((text) => {
    ...
    new import_obsidian23.Setting(contentEl).setName("Quote delimiter").setDesc("Symbols used to mark quotes in your notes. Changes made to selected quotes or symbols will reflect after Obsidian reload.").addDropdown((dropdown) => {
    ...
    new import_obsidian23.Setting(contentEl).setName("Journal default quote tab").setDesc("Choose a journal tab for default display, requires Obsidian reload.").addDropdown((dropdown) => dropdown.addOption("random", "Random").addOption("marked", "Marked").addOption("onThisDay", "On this day").setValue(this.plugin.settings.journalDefaultCurrentMode).onChange(async (value) => {
    ...
    contentEl.createEl("hr");
    new import_obsidian23.Setting(contentEl).setName("Properties").setHeading();
    new import_obsidian23.Setting(contentEl).setName("Show current value").setDesc("Show current property value on list of files filtered by properties.").addToggle((toggle) => toggle.setValue(this.plugin.settings.showCurrentPropertyValue).onChange(async (value) => {
      this.plugin.settings.showCurrentPropertyValue = value;
      await this.plugin.saveSettings();
      this.display();
    }));
    new import_obsidian23.Setting(contentEl).setName("Hide filtered badge").setDesc("Hide the count display of files filtered in properties. The number is based on dropdown choices.").addToggle((toggle) => toggle.setValue(this.plugin.settings.hideFilteredCount).onChange(async (value) => {
  ...
  renderUtilities(contentEl) {
    new import_obsidian23.Setting(contentEl).setName("Utilities").setHeading();
    new import_obsidian23.Setting(contentEl).setName("Plugin icon library").setDesc("Icon library only defines the icons for plugin's native elements.").addDropdown((dropdown) => dropdown.addOption("phosphor", "Phosphor").addOption("lucide", "Lucide").setValue(this.plugin.settings.iconLibrary).onChange(async (value) => {
      this.plugin.settings.iconLibrary = value;
      await this.plugin.saveSettings();
      this.display();
    }));
    contentEl.createEl("hr");
    new import_obsidian23.Setting(contentEl).setName("Backup / restore").setHeading();
    new import_obsidian23.Setting(contentEl).setName("Export settings").setDesc("Export your current portals configuration as a JSON file.").addButton((button) => button.setButtonText("Export").onClick(() => this.exportSettings()));
    new import_obsidian23.Setting(contentEl).setName("Import settings").setDesc("Load settings from a JSON file. This will replace your current configuration.").addButton((button) => button.setButtonText("Import").onClick(() => this.importSettings()));
    new import_obsidian23.Setting(contentEl).setName("Auto backup settings on startup").setDesc("Create timestamped settings backup on Obsidian start. Cloud icon saves it to local storage instead of data.json. This can help prevent sync conflicts on quick switching devices.").addToggle((toggle) => toggle.setValue(this.plugin.settings.enableAutoBackup).onChange(async (value) => {
    ...
    if (this.plugin.settings.enableAutoBackup) {
      new import_obsidian23.Setting(contentEl).setName(" Select Auto backup folder").setDesc("The folder keeps 3 backups, removes older ones. Leave the folder empty to save in the vault root.").addText((text) => text.setPlaceholder("/").setValue(this.plugin.settings.backupFolderPath).onChange(async (value) => {
    ...
    contentEl.createEl("hr");
    new import_obsidian23.Setting(contentEl).setName("Maintenance & help").setHeading();
    new import_obsidian23.Setting(contentEl).setName("Clean up dead portals").setDesc("Remove portal tabs for folders or tags that no longer exist. This cannot be undone.").addButton((button) => button.setButtonText("Clean now").setWarning().onClick(async () => {
      const removed = await this.plugin.cleanupDeadSpaces();
      new import_obsidian23.Notice(removed > 0 ? `Removed ${removed} dead portal(s)` : "No dead portals found");
      this.display();
    }));
    new import_obsidian23.Setting(contentEl).setName("User guide").setDesc("Open the full documentation, the guide covers everything about the plugin.").addButton((button) => button.setButtonText("Open guide").onClick(() => {
      window.open(getGuideUrl(), "_blank");
    }));
    new import_obsidian23.Setting(contentEl).setName("Release notes").setDesc("See the release notes, stay upto date on the latest changes made to the plugin.").addButton((button) => button.setButtonText("Release notes").onClick(() => {
  ...
  async exportSettings() {
    ...
    if (import_obsidian23.Platform.isDesktop) {
      ...
      new import_obsidian23.Notice("Settings exported");
    } else {
      try {
        const existing = this.app.vault.getAbstractFileByPath(fileName);
        if (existing) {
          new import_obsidian23.Notice(`File "${fileName}" already exists. Please rename or delete it first.`);
          return;
        }
        await this.app.vault.create(fileName, data);
        new import_obsidian23.Notice(`Settings exported as ${fileName} in vault root`);
      } catch (err) {
        const message = err instanceof Error ? err.message : String(err);
        new import_obsidian23.Notice(`Export failed: ${message}`);
      }
    }
  }
  importSettings() {
    ...
    input.onchange = async () => {
      ...
      try {
        ...
        await this.plugin.saveSettings();
        this.display();
        new import_obsidian23.Notice("Settings imported successfully");
      } catch (e2) {
        new import_obsidian23.Notice("Invalid settings file");
      }
    };
    input.click();
  }
  ...
  plugin.addCommand({
    ...
    callback: () => {
      ...
      if (leaf && leaf.view instanceof PortalsView) {
        ...
        new import_obsidian24.Notice("Portals view must be open first for all other commands to work.");
      }
    }
  });
  plugin.addCommand({
  ...
  plugin.addCommand({
    ...
    callback: () => {
      ...
      if (rootSpace) {
        ...
        new import_obsidian24.Notice('Vault root portal is not available. Enable "pinned vault" in settings.');
      }
    }
  });
  plugin.addCommand({
    ...
    callback: () => {
      ...
      if (unstacked.length === 0) {
        new import_obsidian24.Notice("All portals are already stacked.");
        return;
      }
  ...
  plugin.addCommand({
    ...
    callback: () => {
      ...
      if (prev) {
        ...
        new import_obsidian24.Notice("No previous portal to switch to.");
      }
    }
  });
  for (let i = 1; i <= 10; i++) {
    plugin.addCommand({
      ...
      callback: () => {
        ...
        if (!space) {
          new import_obsidian24.Notice(`No portal assigned to number ${i}.`);
          return;
        }
        if (space.stackId) {
...
var LucideIconProvider = class {
  ...
  getIconList() {
    if (!this.iconCache) {
      this.iconCache = (0, import_obsidian25.getIconIds)().filter((id) => id.startsWith("lucide-")).map((id) => id.replace("lucide-", "")).sort();
    }
  ...
  renderIcon(element, iconName) {
    const available = (0, import_obsidian25.getIconIds)();
    ...
    (0, import_obsidian25.setIcon)(element, lucideId);
  }
};
...
// src/main.ts
var PortalsPlugin = class extends import_obsidian26.Plugin {
  constructor() {
    super(...arguments);
      ...
      if (this.settings.enableAutoBackup) {
        setTimeout(() => {
          this.performAutoBackup().catch((err) => {
            ...
            new import_obsidian26.Notice("Portals: Auto backup failed - check console.");
          });
        }, 100);
      }
    });
  ...
  async migrateTagNotes() {
    ...
    if (oldPath === newPath) {
      new import_obsidian26.Notice("Old and new folder paths are the same. Nothing to migrate.");
    ...
    if (!(oldFolder instanceof import_obsidian26.TFolder)) {
      new import_obsidian26.Notice(`Old folder "${oldPath}" not found. No notes to migrate.`);
    ...
    const allFiles = oldFolder.children.filter((c) => c instanceof import_obsidian26.TFile && c.extension === "md");
    ...
    if (ignored > 0) {
      new import_obsidian26.Notice(`${ignored} non\u2011tag\u2011note file(s) left in "${oldPath}".`);
    }
  ...
  async cleanupDeadSpaces() {
    ...
    const existingFolders = allFiles.filter((f) => f instanceof import_obsidian26.TFolder).map((f) => f.path);
  ...
  async performAutoBackup() {
    ...
    const srcPath = (0, import_obsidian26.normalizePath)(this.app.vault.configDir + "/plugins/portals/data.json");
    ...
    const normalizedFolder = (0, import_obsidian26.normalizePath)(folderPath);
    if (normalizedFolder !== "/") {
    ...
    const destPath = (0, import_obsidian26.normalizePath)(`${normalizedFolder}/${backupFileName}`);
    try {
